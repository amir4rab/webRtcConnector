...
class WebRtc {
    constructor({ serverUrl, connectionEvent = null, onPeerConnection = null }) {
        ...
        this.sendMessage = (data) => new Promise(async (resolve, reject) => {
            try {
                let encryptedMessage = await aesEncrypt(data, __classPrivateFieldGet(this, _WebRtc_sharedSecret, "f"));
        ...
        this.socket.on('keyExchange', async ({ data: keyExchangeData, from }) => {
            ...
            switch (messageType) {
                case 'offer': {
                    ...
                    await __classPrivateFieldGet(this, _WebRtc_generateKey, "f").call(this);
                    __classPrivateFieldSet(this, _WebRtc_sharedSecret, await ecdhSecretKey(this.selfKeyObj.privateKey, this.peerPublicKey), "f");
        ...
        this.socket.on('message', async ({ encryptedData = null, from }) => {
            ...
            ;
            const deData = await aesDecrypt(encryptedData, __classPrivateFieldGet(this, _WebRtc_sharedSecret, "f"));
